...
export default function AdminTicketsList() {
  ...
  const [items, setItems] = useState<AdminTicketingRecord[]>([]);
  ...
  function startEdit(rec: AdminTicketingRecord) {
  ...
  if (!user) {
    return (
      <p className="text-sm text-zinc-600">Please sign in to view bookings.</p>
    );
  }
  if (user.role !== "admin") {
    return <p className="text-sm text-zinc-600">Admin access required.</p>;
  }

  if (loading) {
    return <p className="text-sm text-zinc-600">Loading all bookings…</p>;
  }

  if (!items.length) {
    return <p className="text-sm text-zinc-600">No bookings found.</p>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {items.map((rec) => {
        ...
        const createdDate = new Date(rec.createdAt).toLocaleDateString();
        return (
          <div
            key={rec._id}
            ...
            <div className="flex flex-col justify-between">
              <div className="space-y-2">
                ...
                <div className="flex items-start gap-2 text-xs text-zinc-600">
                  ...
                  <span>
                    Member: <strong>{rec.user.name}</strong> ({rec.user.email})
                  </span>
                </div>
                <div className="flex items-start gap-2 text-xs text-zinc-600">
                  <span>
